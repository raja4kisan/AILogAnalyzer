...
public class OpenRouterLogAnalyzerService : ILogAnalyzerService
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IConfiguration _configuration;
    private readonly ILogger<OpenRouterLogAnalyzerService> _logger;

    public OpenRouterLogAnalyzerService(
        IHttpClientFactory httpClientFactory,
        IConfiguration configuration,
        ILogger<OpenRouterLogAnalyzerService> logger)
    {
    ...
    public async Task<LogAnalysisResponse> AnalyzeLogsAsync(string logs, string? logType = null, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Starting log analysis with OpenRouter AI");

            // Pre-analyze for common patterns (same as DeepSeek)
            ...
            // Call OpenRouter API
            var aiResponse = await CallOpenRouterAsync(prompt, cancellationToken);
        ...
        {
            _logger.LogError(ex, "Error analyzing logs with OpenRouter");
...
Output ONLY the JSON object.";
    ...
    private async Task<string> CallOpenRouterAsync(string prompt, CancellationToken cancellationToken)
    {
        var apiKey = _configuration["OpenRouter:ApiKey"];
        var baseUrl = _configuration["OpenRouter:BaseUrl"] ?? "https://openrouter.ai/api/v1";
        var model = _configuration["OpenRouter:Model"] ?? "openai/gpt-3.5-turbo";
        ...
        client.DefaultRequestHeaders.Add("HTTP-Referer", "https://github.com/raja4kisan/AILogAnalyzer");
        client.DefaultRequestHeaders.Add("X-Title", "AI Log Analyzer");
    ...
    private LogAnalysisResponse ParseAIResponse(string aiResponse, QuickAnalysis quickAnalysis)
    {
        ...
        if (analysis == null) throw new InvalidOperationException("Failed to deserialize OpenRouter response");
